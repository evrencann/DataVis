...
public class CsvLoader : FileLoader
{
    ...
    public void PrintCsv()
    {
        ...
        int numDataPoints = dataAll.Count;  
        int numBinsSturges = (int)Math.Ceiling(Math.Log(numDataPoints, 2) + 1);
        int numBinsSquareRoot = (int)Math.Ceiling(Math.Sqrt(numDataPoints));
        int CubeRoot = (int)Math.Ceiling(Math.Cbrt(numDataPoints));
        int numBinsRiceRoot = CubeRoot * 2;
        Debug.Log("Number of bins (According to Sturges' rule): " + numBinsSturges);
        Debug.Log("Number of bins (According to Square-root choice): " + numBinsSquareRoot); 
        Debug.Log("Number of bins (According to Rice Root): " + numBinsRiceRoot);
        Debug.Log("Bin size - Interval (According to Sturges' rule): " + (numDataPoints/numBinsSturges)); 
        Debug.Log("Bin size - Interval (According to Square-root choice): " + (numDataPoints/numBinsSquareRoot)); 
        Debug.Log("Bin size - Interval (According to Rice Root): " + (numDataPoints/numBinsRiceRoot));
